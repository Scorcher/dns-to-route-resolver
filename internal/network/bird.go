...
func NewBirdManager(cfg *config.Config) *BirdManager {
	return &BirdManager{
		...
		configPath: cfg.Bird.ConfigPath,
	}
}
...
func (b *BirdManager) Init() error {
	...
	if _, err := os.Stat(b.configPath); os.IsNotExist(err) {
		if err := b.createInitialConfig(); err != nil {
			return fmt.Errorf("failed to create initial BIRD config: %w", err)
		}
	}
...
func (b *BirdManager) AddRoute(nw *net.IPNet) error {
	...
	// Write new config
	if err := os.WriteFile(b.configPath, newConfig, 0644); err != nil {
...
// createInitialConfig creates an initial BIRD configuration file
func (b *BirdManager) createInitialConfig() error {
	// Create a basic BIRD configuration
	config := `# BIRD configuration generated by dns-to-route-resolver
...
# Static routes will be added below this line by dns-to-route-resolver
`

	if err := os.WriteFile(b.configPath, []byte(config), 0644); err != nil {
		return fmt.Errorf("failed to create initial BIRD config: %w", err)
	}

	b.logger.Info("Created initial BIRD configuration at " + b.configPath)
	return nil
}
